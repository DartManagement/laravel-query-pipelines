...
use SimonAnkele\LaravelQueryPipelines\Sortings\SortAscending;
use SimonAnkele\LaravelQueryPipelines\Sortings\SortDescending;
...
it('can sort by multiple columns', function () {
    ...
    $query = TestModel::sort();
    $expected = $query->pluck('id')->toArray();
    $actual = TestModel::query()->orderBy('name')->orderBy('id', 'desc')->pluck('id')->values()->toArray();
    assertEquals($expected, $actual);
});
...
it('can sort ascending by a single column', function (string $column) {
    ...
    $expected = $query->pluck('id')->toArray();
    $actual = TestModel::query()->orderBy($column)->pluck('id')->values()->toArray();
    assertEquals($expected, $actual);
})->with('columns');
...
it('can sort ascending by multiple columns', function () {
    ...
    $expected = $query->pluck('id')->toArray();
    $actual = TestModel::query()->orderBy('name')->orderBy('id')->pluck('id')->values()->toArray();
    assertEquals($expected, $actual);
});
